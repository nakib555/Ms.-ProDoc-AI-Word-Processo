      ...
      return (
        ...
        >
            <button
                ...
                className={`flex items-center w-full px-2 py-1 text-left hover:bg-slate-100 dark:hover:bg-slate-800 rounded-sm group transition-colors gap-2 ${activeMenu === id ? 'bg-slate-200 dark:bg-slate-700 text-slate-900 dark:text-white' : ''}`}
            >
                <Icon size={16} className={`${iconClassName || ''} ${activeMenu === id ? 'text-slate-900 dark:text-white' : 'text-slate-500 dark:text-slate-400 group-hover:text-slate-700 dark:group-hover:text-slate-200'}`} />
   ...
   return (
      ...
      >
          <button
              onClick={(e) => { e.stopPropagation(); !disabled && toggleMenu(id); }}
              onMouseDown={(e) => e.preventDefault()}
              className={`flex flex-col items-center justify-between px-2 py-1.5 min-w-[60px] md:min-w-[68px] h-full rounded-lg transition-all duration-200 group relative text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 ${activeMenu === id ? 'bg-slate-200 dark:bg-slate-700 text-slate-900 dark:text-white shadow-inner ring-1 ring-slate-300 dark:ring-slate-600' : ''}`}
          >
              <div className="flex-1 flex items-center justify-center w-full transition-all mb-0.5">
              ...
              <div className="flex items-center justify-center w-full px-0.5 h-[16px]">
                  ...
                  {hasArrow && <ChevronDown size={10} className={`ml-1 ${activeMenu === id ? 'rotate-180' : ''} transition-transform text-slate-400 group-hover:text-slate-600 shrink-0`} />}
              </div>
          </button>
      </div>
   );
...
export const SmallRibbonButton: React.FC<{ icon: any, label: string, onClick: () => void, className?: string, iconClassName?: string }> = React.memo(({ icon: Icon, label, onClick, className, iconClassName }) => (
  <button 
    onClick={onClick}
    onMouseDown={(e) => e.preventDefault()}
    className={`flex items-center w-full px-2 py-1 text-left hover:bg-slate-100 dark:hover:bg-slate-800 rounded-sm group transition-colors gap-3 ${className || ''}`}
    title={label}
  >
    <Icon size={16} className={`${iconClassName || 'text-slate-500 dark:text-slate-400'} group-hover:text-slate-700 dark:group-hover:text-slate-200 shrink-0`} />
